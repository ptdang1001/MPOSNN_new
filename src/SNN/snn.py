...
def train_test(data, labels, module_name, cur_step, args):
    ...
    training_set, validation_set, testing_set = random_split(data_set, [int(n_samples * 0.6), int(n_samples * 0.2),
                                                                        n_samples - int(n_samples * 0.6) - int(
                                                                            n_samples * 0.2)])
    ...
    save_checkpoint_dir = args.output_dir + "SNN/model_weights_checkpoints/Epoch_" + str(cur_step).zfill(
        3) + '/' + module_name + '/'
...
def predict(data, module_name, cur_step, args):
    ...
    data_grad = torch.tensor(data.values.reshape(n_samples * n_cur_genes), dtype=torch.float,
                             requires_grad=True).reshape(n_samples, n_cur_genes)
    data_grad.retain_grad()
    read_checkpoint_dir = ''
    if args.do_train_snn == 1 and args.do_predict_snn == 1:
        read_checkpoint_dir = args.output_dir + "SNN/model_weights_checkpoints/Epoch_" + str(cur_step).zfill(
            3) + '/' + module_name + "/"
    else:
        read_checkpoint_dir = args.output_dir + "SNN/final_model_weights/" + module_name + "/"
...
def snn(geneExpression, modules_genes, samples_modules_mpo, cur_step, args):
    ...
    if args.do_train_snn == 0 and args.do_predict_snn == 1:
        ...
        try:
            ...
            for module_name in modules:
                ...
                grad_save_path = args.output_dir + 'flux_snn_' + module_name + '_grad_' + str(cur_step).zfill(
                    3) + '.csv'
    ...
    if args.do_train_snn == 1:
        ...
        save_checkpoint_dir = args.output_dir + "SNN/model_weights_checkpoints/" + "Epoch_" + str(cur_step).zfill(3) + '/'
