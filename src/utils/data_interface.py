...
def update_compoundsModules_modulesGenes(compounds_modules, modules_genes):
    all_modules = list(set(list(modules_genes.keys())).intersection(set(compounds_modules.columns.values.tolist())))
    all_modules.sort()
    compounds_modules = compounds_modules[all_modules]
    compounds_modules = remove_outside_compounds(compounds_modules)
    #compounds_modules = make_graph_connected(compounds_modules)
    ...
    #DBG = plot_graph(compounds_modules,'./','M171_V2.png')
    return compounds_modules, modules_genes
...
def load_geneExpression(args):
    ...
    if args.geneExpression_file_name!="TCGA_data_selected_fenton_T_sample4750_gene55411.csv.gz":
...
def save_snn_model_weights(save_step,args):
    ...
    source_dir = args.output_dir + "SNN/model_weights_checkpoints/"+target_folder+'/*'
